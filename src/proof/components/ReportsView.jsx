...
import {
  PROOF_COLORS as C,
  STATUS_META, REQUEST_TYPES, STATUSES,
...
function BarChart({ title, data, colorFn }) {
  ...
  return (
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, padding:20 }}>
      ...
      {entries.length === 0 && <div style={{ fontSize:12, color:C.muted }}>No data yet</div>}
      {entries.map(([k, v]) => {
          ...
          <div key={k} style={{ marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4, alignItems:"center" }}>
              <span style={{ fontSize:12, color:C.text, display:"flex", alignItems:"center", gap:6 }}>
                {colorFn && <span style={{ width:8, height:8, borderRadius:"50%", background:barColor, display:"inline-block", flexShrink:0 }} />}
                {k || "—"}
              </span>
...
export default function ReportsView({ requests, proofreaders }) {
  ...
  // By proofreader — use active proofreader list if available, else derive from data
  const proofreaderNames = proofreaders.length > 0
    ? proofreaders.map(p => p.name)
    : [...new Set(requests.map(r => r.assigned_proofreader).filter(Boolean))].sort();
  const byProofreader = proofreaderNames.reduce((a, name) => ({
    ...a,
    [name]: open.filter(r => r.assigned_proofreader === name).length,
  }), {});

  // By requester / submitted_by
  const byRequester = requests.reduce((a, r) => ({
    ...a,
    [r.submitted_by || "(unknown)"]: (a[r.submitted_by || "(unknown)"] || 0) + 1,
  }), {});

  // By request type
  const byType = REQUEST_TYPES.reduce((a, t) => ({
    ...a, [t]: requests.filter(r => r.request_type === t).length,
  }), {});

  // By client
  const byClient = requests.reduce((a, r) => ({
  ...
  return (
    <div>
      ...
      {/* Charts row 1: status + requester */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
        <BarChart title="Requests by Status"    data={byStatus}    colorFn={statusColor} />
        <BarChart title="Requests by Requester" data={byRequester} />
      </div>

      {/* Charts row 2: proofreader, type, client */}
