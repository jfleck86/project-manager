      ...
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:16 }} onClick={onClose}>
        <div style={{ background:C.card, borderRadius:12, padding:28, maxWidth:600, width:"100%", maxHeight:"90vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:18 }}>
            ...
            <span style={{ fontSize:11, fontWeight:700, color:priorityColor, background:priorityColor+"18", borderRadius:4, padding:"2px 8px" }}>{req.priority}</span>
            <span style={{ fontSize:11, color:C.muted, background:"#f1f5f9", borderRadius:4, padding:"2px 8px" }}>{req.request_type}</span>
            {req.department && <DepartmentBadge department={req.department} />}
            {over && <span style={{ fontSize:11, fontWeight:700, color:"#ef4444", background:"#fee2e2", borderRadius:4, padding:"2px 8px" }}>OVERDUE</span>}
          </div>
