...
export default function QueueView({ requests, proofreaders, onView, onEdit, onQuickAssign }) {
  ...
  // Build proofreader list for filter — from live data, not hardcoded
            ...
            <div key={status}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {items.length===0
                  ...
                  : items.map(r=><RequestCard key={r.id} req={r} onView={onView} onEdit={onEdit} onQuickAssign={onQuickAssign} />)}
              </div>
            </div>
          );
        })}
